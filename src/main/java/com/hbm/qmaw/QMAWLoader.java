...
public class QMAWLoader implements ISelectiveResourceReloadListener {
    ...
    public static void registerJson(String file, JsonObject json) {
        ...
        if(json.has("icon")) {
            qmaw.setIcon(readManualItemStack(json.get("icon").getAsJsonArray()));
        }
        ...
        if(json.has("trigger")) {
            ...
            for(JsonElement element : triggers) {
                ItemStack trigger = readManualItemStack(element.getAsJsonArray());
    ...
    private static ItemStack readManualItemStack(JsonArray array) {
        try {
            ...
            Item item = resolveManualItem(name);
            if(item != null) return new ItemStack(item, stacksize, meta);
        } catch(Exception ignored) {
        }

        MainRegistry.logger.error("Error reading QMAW stack array {} - defaulting to NOTHING item!", array.toString());
        return new ItemStack(ModItems.nothing);
    }

    private static Item resolveManualItem(String name) {
        List<ResourceLocation> candidates = new ArrayList<>();
        ResourceLocation original = toLocation(name);
        if(original != null) {
            addCandidate(candidates, original);
            String path = normalizeManualPath(original.getPath());
            addCandidate(candidates, new ResourceLocation(original.getNamespace(), path));

            if("hbm".equals(original.getNamespace())) {
                addCandidate(candidates, new ResourceLocation("hbmspace", path));
            } else if("hbmspace".equals(original.getNamespace())) {
                addCandidate(candidates, new ResourceLocation("hbm", path));
            }
        }

        for(ResourceLocation candidate : candidates) {
            Item item = getRegisteredItem(candidate);
            if(item != null) return item;
        }

        String fieldName = original == null ? normalizeManualPath(name) : normalizeManualPath(original.getPath());
        Item fieldItem = getFieldItem(ModItemsSpace.class, fieldName);
        if(fieldItem != null) return fieldItem;

        fieldItem = getFieldItem(ModItems.class, fieldName);
        if(fieldItem != null) return fieldItem;

        fieldItem = getFieldItem(ModBlocksSpace.class, fieldName);
        if(fieldItem != null) return fieldItem;

        return getFieldItem(ModBlocks.class, fieldName);
    }
    ...
    private static String normalizeManualPath(String path) {
        if(path.startsWith("item.")) return path.substring("item.".length());
        if(path.startsWith("tile.")) return path.substring("tile.".length());
        return path;
    }

    private static void addCandidate(List<ResourceLocation> candidates, ResourceLocation candidate) {
        if(candidate != null && !candidates.contains(candidate)) {
            candidates.add(candidate);
        }
    }

    private static Item getRegisteredItem(ResourceLocation location) {
    ...
    private static Item getFieldItem(Class<?> holder, String fieldName) {
        try {
