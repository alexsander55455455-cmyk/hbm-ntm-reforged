...
public class BlockPowder extends Block {
    ...
    public boolean canPlaceBlockAt(World world, BlockPos pos) {
        ...
        return block != Blocks.ICE && block != Blocks.PACKED_ICE
                && (block.isLeaves(state, world, pos.down())
                || (block == this && (state.getValue(META) & 7) == 7)
                || (state.isOpaqueCube() && state.getMaterial().blocksMovement()));
    }

    @Override
    public void onEntityWalk(World world, BlockPos pos, Entity entity) {
        if (!world.isRemote && entity instanceof EntityLivingBase entityLiving) {
            entityLiving.addPotionEffect(new PotionEffect(HbmPotion.radiation, 2 * 60 * 20, 14));
        }
    }
