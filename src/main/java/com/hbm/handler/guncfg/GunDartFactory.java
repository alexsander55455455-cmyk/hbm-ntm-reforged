...
public class GunDartFactory {
	...
	public static BulletConfiguration getGPSConfig() {
		...
		bullet.effects.add(new PotionEffect(MobEffects.WITHER, 1200, 2));
		bullet.bHurt = new IBulletHurtBehavior() {
			...
			public void behaveEntityHurt(EntityBulletBase bullet, Entity hit) {
				...
				if(hit instanceof EntityPlayer) {
					...
					if(Library.hasInventoryItem(target.inventory, ModItems.ingot_meteorite_forged)) {
						return;
					}
					...
					if(bullet.shooter instanceof EntityPlayer) {
						...
						for(EnumHand hand : EnumHand.values()) {
							if(shooter.getHeldItem(hand).getItem() == ModItems.gun_darter) {
								ItemGunDart.writePlayer(shooter.getHeldItem(hand), target);
