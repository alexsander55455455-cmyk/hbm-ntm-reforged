    ...
    public void onPlayerLogin(PlayerLoggedInEvent event) {
        if (event.player instanceof EntityPlayerMP player) {
            ...
            if (Loader.isModLoaded(Compat.ModIds.GROOVY_SCRIPT)) {
                sendGroovyRecipeOverrides(player);
            }
        }
    }

    private static void sendGroovyRecipeOverrides(EntityPlayerMP player) {
        try {
            Class<?> container = Class.forName("com.hbm.integration.groovy.HbmGroovyPropertyContainer");
            container.getMethod("sendRecipeOverridesToPlayer", EntityPlayerMP.class).invoke(null, player);
        } catch (ReflectiveOperationException ignored) {
            MainRegistry.logger.warn("GroovyScript integration is not present in this standalone alpha.");
        }
    }
