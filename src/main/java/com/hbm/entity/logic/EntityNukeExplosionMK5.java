...
public class EntityNukeExplosionMK5 extends EntityExplosionChunkloading {
    ...
    public static final double shockSpeed = 2;
    ...
    public void onUpdate() {
        ...
        if (fallout && falloutRain == null) {
            rads = (float) Math.min(10_000_000, Math.pow(radius, 3) * Math.pow(0.5, 2D * this.ticksExisted / radius) + strength);
            if (ticksExisted == 1) {
        ...
        if (!mute) {
        if (radius > 30) {
            this.world.playSound(null, this.posX, this.posY, this.posZ, SoundEvents.ENTITY_LIGHTNING_THUNDER, SoundCategory.AMBIENT,
                    Math.min(1, ticksExisted / 200F) * this.radius * 0.05F, 0.8F + this.rand.nextFloat() * 0.2F);
        } else {
            this.world.playSound(null, this.posX, this.posY, this.posZ, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.AMBIENT,
                    Math.min(1, ticksExisted / 100F) * Math.max(2F, this.radius * 0.1F), 0.8F + this.rand.nextFloat() * 0.2F);
        }
        }
        ...
        if (!explosion.isComplete()) {
            ...
            if (!fallingStarted) {
                if (fallout) {
                    ...
                    falloutRain.setScale((int) ((this.radius * 2.5F + falloutAdd) * BombConfig.falloutRange / 100F), this.radius + 4);
