...
public class GunEMPRay extends Item {
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft) {
        ...
        ArrowLooseEvent event = new ArrowLooseEvent(player, stack, worldIn, charge, Library.hasInventoryItem(player.inventory, ModItems.gun_emp_ammo));
        ...
        if (!infinite && !Library.hasInventoryItem(player.inventory, ModItems.gun_emp_ammo)) {
            return;
        }
        ...
        if (!player.isSneaking()) {
            ...
            EntityDischarge discharge = new EntityDischarge(worldIn, player, 1.0F, hand);
            discharge.setIsCritical(true);
            stack.damageItem(1, player);
            worldIn.playSound(null, player.posX, player.posY, player.posZ, HBMSoundHandler.fatmanShoot, SoundCategory.PLAYERS, 1.0F, 1.0F);
            if (!worldIn.isRemote) {
                worldIn.spawnEntity(discharge);
            }
        } else {
            EntityEMPBlast cloud = new EntityEMPBlast(player.world, 25);
            cloud.posX = player.posX;
            cloud.posY = player.posY + 1.0D;
            cloud.posZ = player.posZ;
            if (!worldIn.isRemote) {
                worldIn.spawnEntity(cloud);
                ExplosionNukeGeneric.empBlast(player.world, player, (int) player.posX, (int) player.posY, (int) player.posZ, 25);
            }
        }
    }
    ...
    public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        ...
        tooltip.add("to shoot ball lightning,");
        tooltip.add("sneak to create EMP wave!");
        tooltip.add("");
        tooltip.add("Ammo: Energy Cell");
        tooltip.add("Damage: 25 - 35");
    }
