...
public class ItemGunOSIPR extends ItemGunBase {
	...
	protected void altFire(ItemStack stack, World world, EntityPlayer player, EnumHand hand) {
		setCharge(stack, 1);
		world.playSound(null, player.posX, player.posY, player.posZ, HBMSoundHandler.osiprCharging, SoundCategory.PLAYERS, 1.0F, 1.0F);
	}
	...
	protected void updateServer(ItemStack stack, World world, EntityPlayer player, int slot, EnumHand hand) {
		...
		if(charge >= 20) {
			EntityCombineBall ball = new EntityCombineBall(player.world, player, 3.0F, hand);
			ball.setDamage(1000.0D);
			world.spawnEntity(ball);
			world.playSound(null, player.posX, player.posY, player.posZ, altConfig.firingSound, SoundCategory.PLAYERS, 1.0F, 1.0F);
			setCharge(stack, 0);
