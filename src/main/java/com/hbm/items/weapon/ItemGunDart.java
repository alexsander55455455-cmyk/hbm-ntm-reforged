...
public class ItemGunDart extends ItemGunBase {
	...
	public ItemGunDart(GunConfiguration config, String name) {
		super(config, name);
	}
	...
	public static void writePlayer(ItemStack stack, EntityPlayer player) {
		...
		stack.getTagCompound().setString("player", player.getName());
	}

	public static EntityPlayer readPlayer(ItemStack stack) {
		if(!stack.hasTagCompound() || FMLCommonHandler.instance().getMinecraftServerInstance() == null) {
			return null;
		}
	...
	public void startAction(ItemStack stack, World world, EntityPlayer player, boolean main, EnumHand hand) {
		...
		EntityPlayer target = ItemGunDart.readPlayer(stack);
		if(target != null) {
			...
			player.sendMessage(new TextComponentString(target.getName()).setStyle(new Style().setColor(TextFormatting.YELLOW)));
