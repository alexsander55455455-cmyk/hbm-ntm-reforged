...
public class BeamPronterSpace {
    private static boolean depthMask = false;

    public static void prontBeamwithDepth(Vec3d skeleton, BeamPronter.EnumWaveType wave, BeamPronter.EnumBeamType beam, int outerColor, int innerColor, int start, int segments, float size, int layers, float thickness, float alpha) {
        depthMask = true;
        prontBeam(skeleton, wave, beam, outerColor, innerColor, start, segments, size, layers, thickness, alpha);
        depthMask = false;
    }

    public static void prontBeam(Vec3d skeleton, BeamPronter.EnumWaveType wave, BeamPronter.EnumBeamType beam, int outerColor, int innerColor, int start, int segments, float size, int layers, float thickness, float alpha) {
        ...
        if (beam == BeamPronter.EnumBeamType.SOLID) {
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            if (beam == BeamPronter.EnumBeamType.LINE && i > 0) {
            ...
            if (beam == BeamPronter.EnumBeamType.SOLID && i > 0) {
        ...
        if (beam == BeamPronter.EnumBeamType.LINE) {
            buffer.begin(GL11.GL_LINE_STRIP, DefaultVertexFormats.POSITION_COLOR);
