...
public class GenLayerEveMountains extends GenLayer {
	
	public GenLayerEveMountains(long seed, GenLayer genLayer) {
		super(seed);
	...
	private int[] getIntsOcean(int p_151626_1_, int p_151626_2_, int p_151626_3_, int p_151626_4_) {
		...
		for(int i1 = 0; i1 < p_151626_4_; ++i1) {
			for(int j1 = 0; j1 < p_151626_3_; ++j1) {
				...
				if(k1 == Biome.getIdForBiome(BiomeGenBaseEve.evePlains)) {
					...
					if(flag) {
						k1 = Biome.getIdForBiome(BiomeGenBaseEve.eveMountains);
					}
				}
