...
public class BiomeGenMoon extends BiomeGenBaseCelestial {
	...
	private BiomeGenMoon(BiomeProperties properties) {
		...
		this.fillerBlock = ModBlocksSpace.moon_rock.getDefaultState();
	}
		...
		for (int l1 = maxHeight - 1; l1 >= 0; --l1) {
			...
			if (l1 <= 0 + rand.nextInt(5)) {
				...
				if (currentState.getBlock() != Blocks.AIR) {
					if (currentState.getBlock() == ModBlocksSpace.moon_rock) {
						if (k == -1) {
							if (l <= 0) {
								topBlockState = Blocks.AIR.getDefaultState();
								fillerBlockState = ModBlocksSpace.moon_rock.getDefaultState();
							} else if (l1 >= 59 && l1 <= 64) {
