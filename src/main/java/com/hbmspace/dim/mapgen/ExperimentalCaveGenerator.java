...
import com.hbmspace.dim.noise.PerlinFestival;
...
public class ExperimentalCaveGenerator extends MapGenBase {
	...
	protected void generateLargeCave(long seed, int chunkX, int chunkZ, ChunkPrimer chunkPrimerIn, double x, double y, double z) {
		this.generateCaveNode(seed, chunkX, chunkZ, chunkPrimerIn, x, y, z, 1.0F + this.rand.nextFloat() * largeCaveSize, 0.0F, 0.0F, -1, -1, 0.5D);
	}

    protected void generateCaveNode(long seed, int chunkX, int chunkZ, ChunkPrimer primer, double x, double y, double z, float caveSize, float yaw, float pitch, int currentStep, int totalSteps, double caveSizeIncrease) {
        ...
        for (boolean flag = random.nextInt(6) == 0; currentStep < totalSteps; ++currentStep) {
            ...
            if (y < depthThreshold) {
                ...
                pitch += (float) (noiseFactor * MathHelper.sin((float) (random.nextFloat() * Math.PI)));
                yaw += (float) (noiseFactor * MathHelper.sin((float) (random.nextInt() * Math.PI)));
            ...
            if (!flag2 && currentStep == k1 && caveSize > 1.0F && totalSteps > 0) {
                this.generateCaveNode(random.nextLong(), chunkX, chunkZ, primer, x, y, z, random.nextFloat() * 0.5F + 0.5F, yaw - ((float) Math.PI / 2F), pitch / 3.0F, currentStep, totalSteps, 1.0D);
                this.generateCaveNode(random.nextLong(), chunkX, chunkZ, primer, x, y, z, random.nextFloat() * 0.5F + 0.5F, yaw + ((float) Math.PI / 2F), pitch / 3.0F, currentStep, totalSteps, 1.0D);
                return;
            }
            ...
            if (flag2 || random.nextInt(4) != 0) {
                ...
                if (x >= centerX - 16.0D - d6 * 2.0D && z >= centerZ - 16.0D - d6 * 2.0D && x <= centerX + 16.0D + d6 * 2.0D && z <= centerZ + 16.0D + d6 * 2.0D) {
                    int i4 = MathHelper.floor(x - d6) - chunkX * 16 - 1;
                    int l1 = MathHelper.floor(x + d6) - chunkX * 16 + 1;
                    int j4 = MathHelper.floor(y - d7) - 1;
                    int i2 = MathHelper.floor(y + d7) + 1;
                    int k4 = MathHelper.floor(z - d6) - chunkZ * 16 - 1;
                    int j2 = MathHelper.floor(z + d6) - chunkZ * 16 + 1;
                                        ...
                                        if (d12 > -0.7D && (d13 * d13 + d12 * d12 + d14 * d14) * (1 + noiseFactor) < 1.0D) {
                                            ...
                                            if ((d13 * d13 + d12 * d12 + d14 * d14) * (1 + noiseValue) < 1.0D + carvingNoise) {
                                                if (Math.pow(d13, 2.5) + Math.pow(d14, 2.2) < 2) {
                                                    digBlock(primer, k2, l3, j3, chunkX, chunkZ, flag1);
                                                }
                                            }
                                            digBlock(primer, k2, l3, j3, chunkX, chunkZ, flag1);
	...
	protected void recursiveGenerate(@NotNull World worldIn, int chunkX, int chunkZ, int originalX, int p_151538_5_, @NotNull ChunkPrimer chunkPrimerIn) {
		...
		for (int j1 = 0; j1 < i1; ++j1) {
			...
			if (this.rand.nextInt(4) == 0) {
				this.generateLargeCave(this.rand.nextLong(), originalX, p_151538_5_, chunkPrimerIn, d0, d1, d2);
			...
			for (int l1 = 0; l1 < k1; ++l1) {
				...
				this.generateCaveNode(this.rand.nextLong(), originalX, p_151538_5_, chunkPrimerIn, d0, d1, d2, f2, f, f1, 0, 0, 1.0D);
			}
		}
	}
    ...
    private boolean isTopBlock(ChunkPrimer primer, int x, int y, int z, int chunkX, int chunkZ) {
        ...
        return primer.getBlockState(x, y, z).getBlock() == biome.topBlock.getBlock();
    }
    ...
    protected void digBlock(ChunkPrimer primer, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
        ...
        if (block == stoneBlock || block == biome.fillerBlock.getBlock() || block == biome.topBlock.getBlock() || state.getMaterial().isLiquid()) {
            if (y < 10) {
