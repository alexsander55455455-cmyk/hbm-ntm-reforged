...
public class TileEntityOrbitalStation extends TileEntityOrbStation implements IFluidStandardReceiver, ITickable {
    ...
    public void update() {
        ...
        if(!world.isRemote) {
            ...
            if(docked != null && (docked.isDead || docked.getState() == EntityRideableRocket.RocketState.UNDOCKING)) {
            ...
            if(docked != null && docked.isReusable()) {
                ...
                if(hasFuel && docked.getState() == EntityRideableRocket.RocketState.NEEDSFUEL) {
                    docked.setState(docked.navDrive != null ? EntityRideableRocket.RocketState.AWAITING : EntityRideableRocket.RocketState.LANDED);
                } else if (!hasFuel && docked.getState() != EntityRideableRocket.RocketState.NEEDSFUEL) {
                    docked.setState(EntityRideableRocket.RocketState.NEEDSFUEL);
                }
            }
